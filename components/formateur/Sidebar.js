...
export function FormateurSidebar() {
    ...
    const navItems = [
        { href: "/formateur", icon: LayoutDashboard, label: "Vue d'ensemble" },
        { href: "/formateur/formations", icon: BookOpen, label: "Mes Formations" },
        { href: "/formateur/apprenants", icon: Users, label: "Apprenants" },
        { href: "/formateur/calendrier", icon: Calendar, label: "Calendrier" },
        { href: "/formateur/evaluations", icon: FileText, label: "Évaluations" },
        { href: "/formateur/messages", icon: MessageSquare, label: "Messages" },
    ];
    ...
    return (
        <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-slate-200 z-50 flex flex-col transition-transform duration-300">
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 mt-auto">
                <Link href="/formateur/parametres" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:text-primary hover:bg-white transition-colors mb-1">
